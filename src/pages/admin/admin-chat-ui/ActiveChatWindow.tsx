  ...
  return (
    <Card className="flex-1 flex flex-col">
      <div className="p-4 border-b flex items-center justify-between flex-shrink-0">
        <h2 className="text-xl font-semibold text-gray-800">{selectedChat.customerName}</h2>
        <div className="flex space-x-2">
          ...
          {!chatIsResolved && (
            <Button variant="destructive" size="sm" onClick={() => onResolveChat(selectedChat.id)}>
      ...
      <ScrollArea className="flex-1 p-4 overflow-y-auto">
        <div className="flex flex-col">
          {selectedChat.messages.map((message) => (
            <ChatMessage key={message.id} message={message} isAdminView={true} />
          ))}
      ...
      <div className="p-4 border-t flex items-center space-x-2 flex-shrink-0">
        <Input
          placeholder={chatIsResolved ? "Chat is resolved. Cannot send messages." : "Type your message..."}
          className="flex-1"
          value={messageInput}
          ...
          disabled={chatIsResolved}
        />
        <Button onClick={onSendMessage} disabled={chatIsResolved}>
